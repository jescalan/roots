...
exports.compile_project = function(cb){
  ...
  var compile_files = function(type, files){
    ...
    type.file_types.forEach(function(file_type){
      ...
      var core_compiler_path = path.join('../lib/compilers/' + file_type + '.js');
      var plugin_compiler_path = path.join(current_directory + 'vendor/plugins' + file_type + '.js');
      ...
      if (fs.existsSync(plugin_compiler_path)) { 
        require(plugin_compiler_path).compile(files[file_type], callback);
      }
