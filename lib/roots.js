...
exports.compile_project = function(root, done){
  ...
  analyze(root).then(compile).then(precompile).then(function(){
    // console.log(global.options.locals);
    compiler.finish();
  }, function(err){
    console.error(err);
  });
...
function analyze(root){
  ...
  function parse_file(file){
    ...
    } else if (global.options.compiled_extensions.indexOf(path.extname(file).slice(1)) >= 0) {
