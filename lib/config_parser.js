...
module.exports = function(args, cb){
  ...
  var opts;
  var config_path = path.join(roots.project.rootDir + '/app.coffee');
  ...
  // compiler-specific configuration (stylus only at the moment)
  if (opts.stylus) { roots.project.compiler_options.stylus = opts.stylus; }

  // set various configurable folders
  roots.project.dirs['views'] = (opts.views_folder || 'views');
  roots.project.dirs['assets'] = (opts.assets_folder || 'assets');
  roots.project.dirs['public'] = (opts.output_folder || 'public');

  opts.locals = opts.locals || {};
  for (var key in opts.locals) {
    roots.project.locals[key] = opts.locals[key];
  }
  for (var key in opts.layouts) {
    roots.project.layouts[key] = opts.layouts[key];
  }

  // add order function to locals
  roots.project.locals.sort = function(ary, opts) {
    opts = opts || {};
  ...
  opts.ignore_folders = opts.ignore_folders.concat([roots.project.dirs['public'], 'plugins']);
  ...
  opts.watcher_ignore_folders = opts.watcher_ignore_folders.concat(['components', 'plugins', '.git', roots.project.dirs['public']]);
  opts.watcher_ignore_files = opts.watcher_ignore_files.concat(['.DS_Store', 'app.coffee']);
  ...
  function format_ignores(ary){
    return ary.map(function(pat){ return "!" + pat.toString(); });
  }

  roots.project.debug = (opts.debug || args.debug); // temporary
