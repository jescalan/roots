...
class FSParser
  ...
  ###*
   * Parses the roots base class' root path, whether file or directory returns
     an "ast" representing the files categorized by the way they need to be
     parsed. The ast is an object with the key being the category name and the
     value being an array of vinyl file instances. It would look something
     like this (with one 'example' extension):
   * {
   *   compiled: [<File>, <File>],
  ...
  ###*
   * Parses the root as a directory, reading it recursively, adding all
     subdirectories to the `dirs` category, and parsing all files with the
   ...
   * Also note the partial application and use of `when/pipeline`. We bind
     each extension and the file to the `sort` function upfront, and leave
     only the last parameter to be set, which represents `extract`, discussed
     below. Pipeline calls each function in an array in order, passing the
     results of the last function to the next one. The arg provided to
     pipeline is what goes to the first function in the list. Detailed docs
     for pipeline found here:
  ...
  ###*
   * Given a file and an extension, runs the extension's `detect` function on
     the file. If it returns false, the file is not added to the extension's
   ...
   * @return {Boolean} promise for a boolean, passed as extract to next
     function
  ...
  ###*
   * Makes sure there are no duplicate directories and that they all
     directories are passed through as vinyl-wrapped file objects.
