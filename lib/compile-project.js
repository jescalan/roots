...
exports.compile_project = function(options, cb){
  ...
  var next = function(counter){
    ...
    config.debug(lang.green + ' compiled'.green);
    if (counter.length < 1) {
  ...
  helpers.create_structure(views, function(files){
    ...
    formats.forEach(function(format){
      require('../lib/compilers/' + format).compile(views.folder, files[format], function(){ next(counter); });
    });
  ...
  helpers.create_structure(assets, function(files){
    ...
    formats.forEach(function(format){
      require('../lib/compilers/' + format).compile(assets.folder, files[format], function(){ next(counter); });
    });
