...
ftp.push_code = function(cb){
  ...
  clear_files('.', function(){
    ...
    readdirp({ root: path.join(process.cwd(), global.options.exportDirectory) }, function(err, res){
      if (err) { return console.error(err); }
  ...
  function put_file(file, cb){
    ...
    client.put(path.join(process.cwd(), global.options.exportDirectory, file), file, cb);
  }
