...
ftp.push_code = function(cb){
  ...
  clear_files('.', function(){
    ...
    readdirp({ root: path.join(roots.project.rootDir, options.output_folder) }, function(err, res){
      if (err) { return console.error(err); }
  ...
  function put_file(file, cb){
    ...
    client.put(path.join(roots.project.rootDir, options.output_folder, file), file, cb);
  }
