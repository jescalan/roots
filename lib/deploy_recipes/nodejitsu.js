...
nodejitsu.add_config_files = function(cb){
  ...
  if (!fs.existsSync(path.join(roots.project.rootDir, 'package.json'))) {
    ...
    var pkg = require(path.join(roots.project.rootDir, 'package.json'));
    pkg.name = pkg.subdomain = this.name === '' ? path.basename(roots.project.rootDir) : this.name;
    fs.writeFileSync(path.join(roots.project.rootDir, 'package.json'), JSON.stringify(pkg));
