...
class Compiler
   ...
   * Creates a new compiler instance, which holds on to the roots instance as
     ...
     object, which is re-created per-compile.
...
class CompileFile
   ...
   * - take each pass over the file, and compile, set compiled content on
     instance
  ...
  ###*
   * Given an array of results from each extension's write_hook, if present,
     figure out how to handle the actual writes, then delegate them to one or
     multiple `write_task`s.
   * - If there are no write hooks, file is written as usual
   * - If a write hook returns false, file is not written regardless of
     anything else
   * - If a write hook returns true, file is written as usual (once at max)
   * - If a write hook returns an object or array of objects with path and
     content props, the file(s) is/are written to the provided path(s) with
     the provided content(s)
  ...
  ###*
   * Single task to write a file. Accepts an optional object with the
     following keys:
   *
   ...
   *
   * If an object is passed, each of these keys is optional, and if not
     provided will be filled in with default values. The path then is wrapped
     with vinyl, passed through the roots output path generator, and the file
     is written. The extension property is only set if there wasn't already an
     extension override and there was a compile, otherwise any extensions are
     preserved as is.
   ...
   * Read the file's extension and grab any and all adapters that match. If
     ...
     just copy the file.
   * If no adapters are found, it's a file with no extension, so it gets a
...
class CompilePass
   ...
   * - First, get the options to be passed in with the adapter, described
     below
   * - Then execute any before pass hooks
   * - Then actually compile, or if no compilation needed just pass the
     content on
   ...
   * This function is responsible for getting all the options together for the
     ...
     see there are 4 different options objects that are merged together to
     make the package of options that are passed in for each file.
   * - global options: set in app.coffee, these are options that are present
     in every file, no matter what
   * - adapter options: also set in app.coffee, these options are specific to
     an adapter. For example, setting `pretty` to `true` for jade only
   * - file options: options that persist only for a single file, for all
     passes
