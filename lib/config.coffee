...
class Config
   ...
   * Creates a new instance of the roots config. This happens once, as soon as
     ...
     default variables, explained below:
   * - output: directory path appended to roots.root that results are written
     to
   ...
   * load_config loads in the app.coffee file, which can overwrite any of
     these previous settings
   * Then, we set up ignores if not provided by app.coffee, and add a couple
     defaults that must always be ignored.
   * Finally, we read the package.json file if present and grab all the
     adapters needed to compile files in this project. More details on that
     later.
   ...
   * This function is responsible for loading the app.coffee file into the
     config.
   * First, it checks the environment. If it's 'development', the default,
     roots loads 'app.coffee', and if there is a custom environment, it tries
     to load 'app.ENV_NAME.coffee'. It then makes sure the config file exists.
     If not, it just returns, and if there was a custom environment logs out a
     warning. If it does exist, there are two ways app.coffee can be
     processed.
   * First is 'simple mode', entered if it doesn't export anything when the
     file is required. In this mode, the config file is processed as a
     coffeescript object. If the file does export anything, this means it's
     being used as a node file, so it is required and processed as a node
     file. Each of the values that are exported are attached directly to the
   ...
   * Given a vinyl-wrapped file and optional extension, this function produces
     the path to the file's destination. To do so, it goes through these
     steps:
