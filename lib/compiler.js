...
Compiler.prototype.copy = function(file){
  ...
  var source = path.join(process.cwd(), file.path);
  var destination = path.join(process.cwd(), 'public', file.path.replace(/^assets|views/, ''));
  var extname = path.extname(file.path).slice(1);
  ...
  global.options.debug.log('copied ' + file.path);
}
