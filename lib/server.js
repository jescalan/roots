...
rootsBrowserAssets = function (req, res, next) {
  var snippet = '<script src="/__roots__/main.js" type="text/javascript">';
...
exports.start = function(serve_dir){
  ...
  if (roots.project.cfg('mode') == 'dev'){
    app.use('/__roots__', connect.static(path.resolve(__dirname, 'browser_assets')));
    app.get('/__roots__/cfg.json', function(req, res){
      // we could expose a bunch of other stuff, I just don't feel like it
      // right now
      res.send(JSON.stringify({livereloadEnabled: roots.project.cfg('livereloadEnabled')}));
    });
