...
var _plugin = function(args){
  ...
  var plugin_folder_path = path.join(roots.project.rootDir, 'plugins');
  ...
  if (cmd === 'generate') {
    ...
    var destination = path.join(roots.project.rootDir, 'plugins/');
  ...
  // install a roots plugin from git repo
    ...
    var plugin_dir = path.join(roots.project.rootDir, 'plugins', repo_string.replace(/.*\//, ""));
