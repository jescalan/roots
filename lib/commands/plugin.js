...
var _plugin = function(args){
  ...
  var plugin_folder_path = path.join(roots.project.root_dir, 'plugins');
  ...
  if (cmd === 'generate') {
    ...
    var destination = path.join(roots.project.root_dir, 'plugins/');
  ...
  // install a roots plugin from git repo
    ...
    var plugin_dir = path.join(roots.project.root_dir, 'plugins', repo_string.replace(/.*\//, ""));
