...
var _watch = function(){
  ...
  roots.project.locals.livereload = socket_script + spinner_html;
  ...
  roots.compile_project(roots.project.rootDir, function(){
    server.start(roots.project.path('public'));
  });
    ...
    if (fs.existsSync(file.fullPath)){
      ...
      for (var i = 0; i < ignored.length; i++){
        if (minimatch(path.basename(file.path), ignored[i].slice(1))) {
          roots.print.debug('ignored file changed, reloading project');
